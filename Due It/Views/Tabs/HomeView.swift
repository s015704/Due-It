...
struct HomeView: View {
    ...
    var body: some View {
        ZStack{
            ...
            VStack{
                ...
                HStack {
                    Button(action: {
                        for j in (0..<self.curAssignments.count).reversed() {
                            if self.curAssignments[j].timeLeft<0.01 {
                                self.curAssignments.remove(at: j)
                            }
                        }
                        //self.getWorkForWeek()
                    }){Text("Next Day")}
    ...
    func getAllTimePerDay(_ date: Date) -> [(Int, Double)] {
        ...
        for i in 0..<curAssignments.count {
            if daysBetween(date, curAssignments[i].dueDate)<6&&daysBetween(date, curAssignments[i].dueDate)>0&&curAssignments[i].timeLeft>0.01 {
